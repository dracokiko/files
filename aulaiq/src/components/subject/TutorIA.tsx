// TODO: Later this will connect to the AI/RAG backend using subject-specific materials,
// professor corrections, slides and exam criteria.
...
const DEMO_AI_RESPONSE = (subject: Subject, userMsg: string): string => {
  const greetings = [
  ...
  function sendMessage(text: string) {
    ...
    setTimeout(() => {
      const aiMsg: Message = {
        ...
        role: 'ai',
        content: DEMO_AI_RESPONSE(subject, text),
      };
    ...
    <div className="flex flex-col h-[70vh] max-h-[600px]">
      ...
      {!isPaid && (
        <div className={`flex items-center justify-between px-1 py-2 text-xs mb-2 ${
          limitReached ? 'text-red-500' : 'text-gray-400'
        }`}>
          <span>
            {limitReached
              ? 'Limite diário atingido. Faz upgrade para continuar.'
              : `Mensagens hoje: ${msgCount}/${MAX_FREE_MESSAGES}`}
          </span>
          {limitReached && (
            <button
