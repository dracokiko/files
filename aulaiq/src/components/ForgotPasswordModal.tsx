...
export default function ForgotPasswordModal({ onClose, onSubmit, onBackToLogin }: ForgotPasswordModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!email) {
      setError('Preenche o teu email.');
      return;
    }

    setLoading(true);
    const submitError = await onSubmit(email.trim().toLowerCase());
    setLoading(false);
      ...
      <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-md animate-slide-up overflow-hidden">
        ...
        <div className="bg-gradient-to-br from-blue-600 to-violet-600 px-8 pt-8 pb-6">
          ...
          <p className="text-blue-100 text-sm mt-1">
            {sent ? 'Verifica o teu email.' : 'Enviamos-te um link para criares uma nova password.'}
          </p>
        </div>
          ...
          <div className="px-8 py-6 space-y-4">
            <div className="flex items-start gap-2 p-3 bg-green-50 border border-green-100 rounded-xl">
              ...
              <p className="text-xs text-green-700">
                Se existir uma conta com o email <strong>{email}</strong>, vais receber um link para repor a password.
              </p>
            </div>
            <button
              type="button"
